...
## `--skip-isolators-tag` does not exist in 6.1

Older Oro documentation and community posts reference `--skip-isolators-tag`, but the 6.1 CLI rejects it as an unknown option. The flag that exists is `--skip-isolators`, which kills *all* isolation and is meant for e2e runs against a deployed app; leave isolation on for local integration runs.
